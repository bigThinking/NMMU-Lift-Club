 ...
 * @author s210036575
 */
public class RideResourceClient implements URL{
...
//private static final String BASE_URI = "http://192.168.5.125:8060/lift_club";
    ...
    public RideResourceClient(String authToken, Client client) {
      //  if (client == null)
            this.client = javax.ws.rs.client.ClientBuilder.newBuilder().build();//.sslContext(getSSLContext())
      //  else this.client = client;

//        webTarget = this.client.target(BASE_URI).path("ride");
        this.authToken = authToken;
    }

    public String getRides() throws ClientErrorException {
        webTarget = this.client.target(BASE_URI).path("ride/rides");
        Response result = addHeaders(webTarget.request(javax.ws.rs.core.MediaType.APPLICATION_JSON)).get(Response.class);
    ...
    public String getMatchs() throws ClientErrorException {
        webTarget = this.client.target(BASE_URI).path("ride/matches");
        Response result = addHeaders(webTarget.request()).get(Response.class);
